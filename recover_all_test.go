...
func TestRunRestoreAllConsumesPayloadFetchSourcesWithOpaqueStorageRefs(t *testing.T) {
	// This test verifies the core bug fix: when file.StorageRefs contain opaque
	// (non-fetchable) values but payload.FetchSources has valid source entries,
	// restore-all should use FetchSources and succeed instead of returning
	// UNSUPPORTED_FETCH_SOURCE.
	now := time.Date(2026, 3, 24, 8, 0, 0, 0, time.UTC)
	tempDir := t.TempDir()
	privateKey := mustPrivateKey(t, "1111111111111111111111111111111111111111111111111111111111111111")
	vaultOwner := strings.ToLower(crypto.PubkeyToAddress(privateKey.PublicKey).Hex())

	fileContent := []byte("restored via fetch_sources v1 path")
	...
	ciphertext := mustSealAESGCM(t, fileKeyBytes, contentIV, fileContent)
	encPath := filepath.Join(tempDir, "enc-source.enc")
	...
	encFileURL := (&url.URL{Scheme: "file", Path: encPath}).String()

	payload := recoveryMapPayload{
		Snapshot: &recoveryPackageSnapshot{
			SchemaVersion:         recoveryPackageSchemaVersion,
			PackageID:             "test-fetch-sources-package",
			MapID:                 "test-fetch-sources-package",
		...
		FileIndex: []recoveryMapFileIndex{
			{
				FileID:   fileID,
				FileName: "video.mp4",
				Name:     "media/video.mp4",
				Size:     int64(len(fileContent)),
				CID:      "",
				// Key point: StorageRefs are OPAQUE — not fetchable URLs.
				StorageRefs: []recoveryMapStorageRef{
					{Kind: "provider_operation_ref", Value: "op_opaque_not_a_url"},
				},
				...
				RecoveryMaterialVersion: &matVersion,
			},
		},
		// FetchSources has the actual fetchable source — a file:// URL.
		FetchSources: []recoveryPackageFetchSource{
			{
				FileID:                 fileID,
				SourceType:             "provider_piece_url",
				SourceRef:              encFileURL,
	...
	ct, tag, err := encryptPayload(doc, payload, key)
	if err != nil {
		t.Fatalf("encrypt payload: %v", err)
	}
	doc.PayloadCiphertext = ct
	doc.PayloadTag = tag

	qrmPath := filepath.Join(tempDir, "restore-fetch-sources.qrm")
	writeTestQRM(t, qrmPath, doc)
	...
	output := stdout.String()
	if !strings.Contains(output, "restore_all_complete=true") {
		t.Fatalf("expected restore_all_complete=true, got:\n%s", output)
	}
	if !strings.Contains(output, "files_restored=1") {
		t.Fatalf("expected files_restored=1, got:\n%s", output)
	}
	if strings.Contains(output, "UNSUPPORTED_FETCH_SOURCE") {
		t.Fatalf("BUG NOT FIXED: got UNSUPPORTED_FETCH_SOURCE despite valid FetchSources:\n%s", output)
	}
	// Verify the source came from FetchSources, not StorageRefs
	if !strings.Contains(output, "source=provider_piece_url:file://") {
		t.Fatalf("expected source resolved from FetchSources, got:\n%s", output)
	}
	...
	if !found {
		t.Fatalf("expected media/video.mp4 in zip")
	}
}

func TestRunRestoreAllResolvesBareCIDViaEnvPieceGateway(t *testing.T) {
	// This test simulates the real production scenario:
	// - FetchSources have source_type="cid" and source_ref=<bare piece CID>
	// - StorageRefs have only opaque refs (bucket, dataset, operation) — NO provider_piece_url
	// - The piece gateway is available via QAVE_FETCH_PIECE_BASE_URL env var
	// - restore-all should read the env var and use it to build gateway URLs
	now := time.Date(2026, 3, 24, 9, 0, 0, 0, time.UTC)
	tempDir := t.TempDir()
	privateKey := mustPrivateKey(t, "1111111111111111111111111111111111111111111111111111111111111111")
	vaultOwner := strings.ToLower(crypto.PubkeyToAddress(privateKey.PublicKey).Hex())

	fileContent := []byte("production-style cid-based restore content")
	fileKeyBytes := bytes.Repeat([]byte{0x21}, fileKeyLengthBytes)
	wrapIV := bytes.Repeat([]byte{0x56}, wrapIVLengthBytes)
	contentIV := bytes.Repeat([]byte{0x34}, wrapIVLengthBytes)
	saltBytes := bytes.Repeat([]byte{0x78}, 16)
	...
	// Serve ciphertext from a local HTTP server simulating the piece gateway
	pieceCID := "bafybeigdyrzt5sfp7udm7hu76uh7y26nf3efuylqabf3okuber"
	mux := http.NewServeMux()
	mux.HandleFunc("/piece/"+pieceCID, func(w http.ResponseWriter, r *http.Request) {
	...
	// Set the env var (restore-all now reads this)
	oldEnv := os.Getenv(pieceGatewayBaseURLEnv)
	os.Setenv(pieceGatewayBaseURLEnv, gatewayBase)
	defer func() {
		if oldEnv != "" {
	...
	payload := recoveryMapPayload{
		Snapshot: &recoveryPackageSnapshot{
			SchemaVersion:         recoveryPackageSchemaVersion,
			PackageID:             "test-cid-gateway",
			MapID:                 "test-cid-gateway",
			VaultOwner:            vaultOwner,
		...
		FileIndex: []recoveryMapFileIndex{
			{
				...
				Size:     int64(len(fileContent)),
				CID:      pieceCID,
				// Production-style: only opaque refs, NO provider_piece_url
				StorageRefs: []recoveryMapStorageRef{
					{Kind: "bucket", Value: "user-vault"},
					{Kind: "provider_dataset_id", Value: "ds-123"},
					{Kind: "provider_operation_ref", Value: "op-abc"},
				},
				UploadedAt:    now.Format(time.RFC3339Nano),
				SnapshotIndex: 0,
				ExpiresAt:     "2099-01-01T00:00:00Z",
		...
		// Production-style FetchSources: source_type="cid", source_ref=bare CID
		FetchSources: []recoveryPackageFetchSource{
			{
				FileID:                 fileID,
				SourceType:             "cid",
				SourceRef:              pieceCID,
				PieceRef:               stringPtr(pieceCID),
				CID:                    stringPtr(pieceCID),
				FetchCapabilityVersion: "qave.recovery-fetch.v1",
			},
		},
		WrappedKeys: []recoveryPackageWrappedKey{
	...
	qrmPath := filepath.Join(tempDir, "restore-cid-gateway.qrm")
	writeTestQRM(t, qrmPath, doc)

	var stdout bytes.Buffer
	err = run(
		// Note: NOT passing --piece-base-url — relying on env var
		[]string{"restore-all", qrmPath, "--signer", "manual", "--no-browser"},
		&stdout,
		ioDiscard{},
		buildRestoreAllStdin(t, doc, privateKey, recoveryKey),
	...
	if err != nil {
		t.Fatalf("restore-all with CID+gateway failed: %v\nstdout: %s", err, stdout.String())
	}
	...
	// Verify it used the piece gateway
	if !strings.Contains(output, "source=piece_gateway:"+pieceCID) {
		t.Fatalf("expected source to be piece_gateway, got:\n%s", output)
	}
}
